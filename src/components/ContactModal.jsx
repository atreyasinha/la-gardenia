...
export default function ContactModal({ isOpen, onClose }) {
  ...
  const sendWhatsApp = () => {
    ...
    const text = `Hi La Gardenia (Chas, Bokaro), I would like to inquire about booking an event!\n\n*Name:* ${formData.name}\n*Phone:* ${formData.phone}\n*Event Date:* ${formData.eventDate}\n*Event Type:* ${formData.eventType}\n*Guest Count:* ${formData.guests}\n*Venue Space:* ${formData.venueSpace}\n*Notes:* ${formData.notes}`;
    const encodedText = encodeURIComponent(text);
    // SECURITY: Add noopener,noreferrer to prevent reverse tabnabbing
    window.open(`https://wa.me/919431911929?text=${encodedText}`, '_blank', 'noopener,noreferrer');
  };
